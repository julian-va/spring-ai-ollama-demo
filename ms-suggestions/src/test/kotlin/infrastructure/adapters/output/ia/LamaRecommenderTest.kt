...
class LamaRecommenderTest {
    ...
    fun `should return concatenated text from chat responses`() = runTest {
        ...
        given(chatModel.stream(org.mockito.ArgumentMatchers.any(Prompt::class.java)))
            .willReturn(Flux.just(response1, response2))

        // When
        val result = recommender.recommend(messageSuggestion).first()
        ...
        verify(chatModel).stream(org.mockito.ArgumentMatchers.any(Prompt::class.java))
    }
}